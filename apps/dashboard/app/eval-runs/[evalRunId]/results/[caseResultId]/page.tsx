...
export default async function EvalCaseDetailPage({ params }: EvalCaseDetailPageProps) {
  ...
  return (
    <main style={{ padding: '48px', maxWidth: '1120px', margin: '0 auto' }}>
      <Link href={`/eval-runs/${evalRunId}`} style={{ color: '#475569', textDecoration: 'none' }}>
        ← Eval run detail
      </Link>
      ...
      {!result.ok ? (
        ...
        <>
          <h1 style={{ marginTop: '12px', fontSize: '44px', lineHeight: 1.05 }}>
            {result.result.question || result.result.test_case_id}
          </h1>
          <p style={{ fontSize: '18px', color: '#475569', lineHeight: 1.6 }}>
            Status <strong>{result.result.status}</strong> · Verdict{' '}
            <strong>{result.result.scores.verdict}</strong> · Failure{' '}
          ...
          <section className="panel">
            <dl className="metric-grid">
              ...
              <div>
                <dt>Cost</dt>
                <dd>{formatCurrency(result.result.cost_usd)}</dd>
              </div>
              <div>
                <dt>Hit@5</dt>
                <dd>{result.result.scores.retrieval.hit_at_5 ? 'yes' : 'no'}</dd>
              </div>
              <div>
                <dt>Recall@10</dt>
                <dd>{formatPercent(result.result.scores.retrieval.recall_at_10)}</dd>
              </div>
              <div>
                <dt>Citation validity</dt>
                <dd>{formatPercent(result.result.scores.citations.citation_validity)}</dd>
              </div>
              <div>
                <dt>Citations</dt>
                <dd>{result.result.scores.citations.citation_count}</dd>
              </div>
            </dl>
          </section>
          ...
          <section className="answer-grid">
            <section>
              ...
              <p>{result.result.expected_answer || 'No expected answer recorded.'}</p>
            </section>
            <section>
              <h2>Generated answer</h2>
              <p>{result.result.answer || 'No generated answer recorded.'}</p>
            </section>
          </section>

          <section className="source-comparison-grid">
            <article className="panel">
              <h2>Expected sources</h2>
              <p>{joinList(getExpectedSources(result.result))}</p>
            </article>
            <article className="panel">
              <h2>Retrieved expected sources</h2>
              <p>{joinList(result.result.scores.retrieval.retrieved_expected_sources)}</p>
            </article>
            <article className="panel error-panel">
              <h2>Missing expected sources</h2>
              <p>{joinList(result.result.scores.retrieval.missing_expected_sources)}</p>
            </article>
            <article className="panel error-panel">
              <h2>Invalid citations</h2>
              <p>{joinList(result.result.scores.citations.invalid_citations)}</p>
            </article>
          </section>
          ...
          {result.result.error_message ? (
            <section className="panel error-panel">
              ...
              <p>{result.result.error_message}</p>
            </section>
          ) : null}

          {result.result.judge_error ? (
            <section className="panel error-panel">
              <h2>Judge error</h2>
              <p>{result.result.judge_error}</p>
            </section>
          ) : null}
          ...
          {result.result.judge ? (
            <section className="judge-panel">
              ...
              <dl className="metric-grid">
                <div>
                  <dt>Groundedness</dt>
                  <dd>{formatPercent(result.result.judge.scores.groundedness)}</dd>
                </div>
                <div>
                  <dt>Correctness</dt>
                  <dd>{formatPercent(result.result.judge.scores.correctness)}</dd>
                </div>
                <div>
                  <dt>Completeness</dt>
                  <dd>{formatPercent(result.result.judge.scores.completeness)}</dd>
                </div>
                <div>
                  <dt>Citation support</dt>
                  <dd>{formatPercent(result.result.judge.scores.citation_support)}</dd>
                </div>
              </dl>
              ...
              <p>
                ...
                {joinList(result.result.judge.missing_important_points)}
              </p>
            </section>
          ) : null}
        </>
